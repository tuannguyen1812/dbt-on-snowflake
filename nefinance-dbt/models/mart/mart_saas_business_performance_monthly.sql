{{ config(
    ...
    unique_key = 'business_performance_month_key',
    incremental_strategy = 'merge',
    on_schema_change = 'sync_all_columns',
    cluster_by = ['performance_month']
) }}

with revenue as (

    select * from {{ ref('fct_account_revenue_monthly') }}
    {% if is_incremental() %}
        where revenue_month >= (
...
usage_monthly as (
    ...
    from {{ ref('fct_feature_usage_daily') }}
    {% if is_incremental() %}
        where usage_date >= (
            select dateadd(month, -2, coalesce(max(performance_month), to_date('1900-01-01')))
            from {{ this }}
