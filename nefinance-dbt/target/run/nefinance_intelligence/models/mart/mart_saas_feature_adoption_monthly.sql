        ...
        create or replace transient table NEFINANCE_DB.PROD.mart_saas_feature_adoption_monthly
         as
        (select * from (
...
left join features
    on monthly.feature_key = features.feature_key
              ) order by (usage_month, feature_name)
        );
      alter  table NEFINANCE_DB.PROD.mart_saas_feature_adoption_monthly cluster by (usage_month, feature_name);
